...
use crate::model::ParseItemError;
use std::io::Read;
use std::io::Write;
...
impl std::fmt::Display for StorageError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            StorageError::ItemNotFound(id) => write!(f, "item not found: {}", id),
        }
    }
}
...
/// get item by id
pub fn get_item_by_id(id: u32)  -> Result<Item, StorageError>{
    let items = get_all()?;
    items
        .iter()
...
/// update item
pub fn update_item(updated: Item) -> Result<(), StorageError> {
    let items = get_all()?;
    let path = get_csv_path()?;
    let path = Path::new(&path);
    ...
    for item in items {
        if item.id() == updated.id() {
            writeln!(file, "{}", updated.to_string())?;
        } else {
            writeln!(file, "{}", item.to_string())?;
        }
    }
